{% macro vd_recon_aggregate_detail_evidence(relation, mismatch_table_ref, group_filter, candidate_expressions, mapped_columns, log_result=true) %}
  ...
  {# VD-4529: mismatch_table_ref is adapter-quoted as of the materialize macros' relation-API
     rendering, and information_schema stores identifiers unquoted, so the parts must be stripped
     before they are compared. Routed through the one shared parser rather than stripping here —
     and rather than reading target.database/target.schema, which would hardcode an invariant this
     macro's own signature does not enforce: it accepts any caller-supplied ref, including refs
     naming a relation it did not create. #}
  {% set ref_parts = vd_recon_relation_parts(mismatch_table_ref) %}
  {% set probe_query %}
