{% macro vd_recon_materialize_keyed_mismatches(baseline_relation, target_relation, key_columns, column_pairs, run_id, carry_columns=[], source_relations=[]) %}
  ...
  {# VD-4529: the ephemeral workspace catalog is named ephm_<domain-slug>_<intent-slug> and those
     slugs routinely contain hyphens, which an unquoted identifier parses as subtraction. Build the
     name through dbt's relation API rather than interpolating a dotted string, so the adapter
     supplies the quote character and its escaping rules and this does not need re-fixing per
     platform. Never write a quote character here.

     quote_policy is passed explicitly rather than inherited: api.Relation.create merges the
     PROJECT's `quoting:` config over the all-true default, so a consumer project configured with
     `quoting: {database: false}` would otherwise silently reintroduce this exact bug. #}
  {% if not run_id is string or not modules.re.match('^[A-Za-z0-9_]+$', run_id) %}
    {{ exceptions.raise_compiler_error("vd_recon_materialize_keyed_mismatches: run_id must match ^[A-Za-z0-9_]+$ — it becomes a quoted SQL identifier, and a '.' or '\"' inside it would silently corrupt the mismatch table's name and every downstream parse of it. Got: " ~ run_id) }}
  {% endif %}
  {% set mismatch_table = api.Relation.create(
       database=target.database,
       schema=target.schema,
       identifier='vd_recon_mismatch_' ~ run_id,
